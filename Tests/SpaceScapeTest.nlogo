...
abstract-nodes-own [vector integration ]
abstract-links-own [abstract-length]

patches-own [ population country-name elevation ]
turtles-own [ name country-color country-VectorFeature]
...
to setup-bikes
  ...
  ;gis:load-coordinate-system ("/Users/Juste/Documents/Complex Systems/SustainableDistrict/Data/Stockholm/Sodermalm/sodemalm/BikeLanes.prj")
  ; Load all of our datasets
  ;set bike-dataset gis:load-dataset "/Users/Juste/Documents/Complex Systems/SustainableDistrict/Data/Stockholm/Sodermalm/sodemalm/sodemalm_lin.shp"
  ; Set the world envelope to the union of all of our dataset's envelopes
  gis:set-world-envelope (gis:envelope-of bike-dataset)
                                           
end

to display-bikes
  gis:set-drawing-color red
  gis:draw bike-dataset 1
  let i 0
  foreach gis:feature-list-of bike-dataset
    [ show gis:vertex-lists-of ?
      if i mod 8 = 0 [
      ...
      [ create-river-labels 1
          [ set xcor item 0 centroid
            ...
            set size 0
            set label gis:property-value ? "ST_NAME" ] ] ]
...
to setup-soldermam
  ...
  ;gis:load-coordinate-system ("/Users/Juste/Documents/Complex Systems/SustainableDistrict/Data/Stockholm/Sodermalm/sodemalm/BikeLanes.prj")
  ; Load all of our datasets
  ;set dataset gis:load-dataset "/Users/Juste/Documents/Complex Systems/SustainableDistrict/Data/Bergsjon/GIS/Bergsjon/Bergsjon.shp"
  set dataset gis:load-dataset "/Users/Juste/Documents/Complex Systems/SustainableDistrict/Data/Stockholm/Sodermalm/sodermalm2/sodermalm2_lin.shp"
  ; Set the world envelope to the union of all of our dataset's envelopes
...
to extract-abstract-graph-random-pos
  foreach gis:feature-list-of dataset [create-abstract-nodes 1 [setxy random-xcor random-ycor set shape "circle" set color red set hidden? true set vector ?]]
  ask abstract-nodes [let v vector ask other abstract-nodes [if gis:intersects? vector v and not abstract-link-neighbor? myself [let vv vector create-abstract-link-with myself [set abstract-length gis:property-value v "LENGTH" + gis:property-value vv "LENGTH" set hidden? true set color orange]]]]
end
...
to calculate-spatial-integration
  ...
  ask abstract-nodes [
    set integration 0
    ask other abstract-nodes [ifelse nw:weighted-distance-to myself "abstract-length" != false [set integration integration + nw:weighted-distance-to myself "abstract-length"][set integration integration + 100]] 
  ]
  let mi min [integration] of abstract-nodes
  let ma max [integration] of abstract-nodes
  ask abstract-nodes [gis:set-drawing-color scale-color red (- integration)  (- (ma + 50)) (- (mi - 50 ))  gis:draw vector 3]
end

to test-spatial-integration
  setup-soldermam
  extract-abstract-graph-random-pos
...
to color-countries
;  foreach gis:feature-list-of countries-dataset [
;     gis:set-drawing-color random 140
;     gis:fill ? 1 
;  ]
  ;;color sea with patches?
  ask patches[
     foreach gis:feature-list-of countries-dataset [
       if not gis:intersects? ? self [
         set pcolor blue
       ]
     ]
  ]

;  let patches-data gis:patch-dataset population
;  ask patches gis:intersecting countries-dataset [set pcolor green]

;;Test 4colors Theorem
  foreach gis:feature-list-of countries-dataset [
    set forbidden-colors []
    ask turtles with [length name > 0 and name != gis:property-value ? "CNTRY_NAME"] [
      if gis:intersects? country-VectorFeature ? [
        set forbidden-colors fput country-color forbidden-colors
        show word "neighbor:" name
      ]

    ]
    show gis:property-value ? "CNTRY_NAME"
    show forbidden-colors
    show count turtles
    
    ifelse not member? red forbidden-colors [gis:set-drawing-color red]
    [ifelse not member? green forbidden-colors [gis:set-drawing-color green]
      [ifelse not member? pink forbidden-colors [gis:set-drawing-color pink]
        [if not member? yellow forbidden-colors [gis:set-drawing-color yellow]]
        ]
      ]
    gis:fill ? 1 
    
    if count turtles with [name = gis:property-value ? "CNTRY_NAME"] = 0 [create-turtles 1 [set country-color gis:drawing-color set country-VectorFeature ? set name gis:property-value ? "CNTRY_NAME"]]
  ]
